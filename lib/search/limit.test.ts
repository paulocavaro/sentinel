...
describe('takeToken', () => {
  it('allows the first question', () => {
    expect(takeToken('1.2.3.4', 0)).toEqual({ ok: true })
  })
  ...
  it('allows up to MAX_IN_WINDOW', () => {
    ...
    for (let i = 0; i < MAX_IN_WINDOW; i += 1) {
      expect(takeToken('1.2.3.4', i * 60_000)).toEqual({ ok: true })
    }
  })

  it('refuses the one after that, and says how long to wait', () => {
    drain('1.2.3.4', MAX_IN_WINDOW, 0)
    ...
    expect(takeToken('1.2.3.4', 60_000)).toEqual({
      ok: false,
  ...
  it('allows again once the window has passed', () => {
    drain('1.2.3.4', MAX_IN_WINDOW, 0)
    expect(takeToken('1.2.3.4', WINDOW_MS - 1)).toMatchObject({ ok: false })

    // The instant the first ten fall out of the window, the eleventh is a first
    // question again.
    expect(takeToken('1.2.3.4', WINDOW_MS)).toEqual({ ok: true })
  })

  it('counts each key separately', () => {
    drain('1.2.3.4', MAX_IN_WINDOW, 0)

    expect(takeToken('1.2.3.4', 0)).toMatchObject({ ok: false })
    expect(takeToken('5.6.7.8', 0)).toEqual({ ok: true })
  })
  ...
  it('does not count a refused question, so hammering cannot extend the wait', () => {
    ...
    for (let i = 0; i < 50; i += 1) takeToken('1.2.3.4', WINDOW_MS / 2)
  ...
  it('never says to come back in zero seconds', () => {
    ...
    expect(takeToken('1.2.3.4', WINDOW_MS - 1)).toEqual({ ok: false, retryAfter: 1 })
  })
  ...
  it('keeps a key that is still inside its window when it sweeps', () => {
    ...
    // second.
    takeToken('9.10.11.12', WINDOW_MS + 1)
    expect(bucketCount()).toBe(2)
  ...
  it('reads the wall clock when no instant is given', () => {
    ...
    for (let i = 0; i < MAX_IN_WINDOW; i += 1) {
      expect(takeToken('1.2.3.4')).toEqual({ ok: true })
    }

    expect(takeToken('1.2.3.4')).toMatchObject({ ok: false })
  })
})
...
describe('bucketKey', () => {
  ...
  it('falls back to the first entry of x-forwarded-for', () => {
    ...
    const headers = new Headers({ 'x-forwarded-for': '198.51.100.4, 70.41.3.18, 150.172.238.178' })

    expect(bucketKey(headers)).toBe('198.51.100.4')
  })

  it('trims the space after the comma that proxies conventionally write', () => {
    const headers = new Headers({ 'x-forwarded-for': '  198.51.100.4  ,  70.41.3.18' })
